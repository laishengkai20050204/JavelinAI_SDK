...
export default function AdminConfigConsole() {
  ...
  const restoreDefaults = async () => {
    if (!window.confirm("Restore defaults? This will clear all runtime overrides.")) return;
    setResetting(true);
    setError(null);
    setOkMsg(null);
    try {
  ...
  const addToggle = () => {
    const name = window.prompt("Enter tool name (function name)");
    if (!name) return;
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-10 border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-slate-800 dark:bg-slate-900/80 dark:supports-[backdrop-filter]:bg-slate-900/60">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold leading-tight">Javelin Admin Console</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">Runtime config · Hot reload · Key masking</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              title="Trigger a reload broadcast"
            >
              <RefreshCw size={16} /> {reloading ? "Reloading..." : "Reload"}
            </button>
            <button
              onClick={load}
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        <div className="space-y-2 mb-4">
          {loading && (<Banner icon={<RefreshCw className="animate-spin" size={16} />} text="Loading config" color="slate" />)}
        ...
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <Section title="Network & timeouts (optional overrides)">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Field label="API Key (never shows old)">
                <div className="relative">
                  <input
                    value={newApiKey}
                    onChange={(e) => setNewApiKey(e.target.value)}
                    placeholder={apiKeyMasked ? `Current: ${apiKeyMasked}` : "Not set"}
          ...
          <Section title="Tool toggles">
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">Unspecified tools default to enabled (true)</span>
              </div>
              <div className="grid gap-2 md:grid-cols-2">
                {Object.keys(toolToggles).length === 0 && (
                  <div className="text-sm text-slate-500 dark:text-slate-400">(No explicit toggles)</div>
                )}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium ${saving || !diffPayload ? "bg-slate-300 text-white dark:bg-slate-700" : "bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400"}`}
            >
            ...
            >
              <RefreshCw size={16} /> Undo unsaved
            </button>
            <button
            ...
            >
              <Trash2 size={16} /> {resetting ? "Restoring..." : "Restore defaults"}
            </button>
            <button
              onClick={() => setShowDiff(!showDiff)}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"
            >
              {showDiff ? <EyeOff size={16} /> : <Eye size={16} />} {showDiff ? "Hide changes" : "Show payload"}
            </button>
          </div>

          {showDiff && (
            <motion.pre initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 rounded-xl bg-slate-900 text-slate-100 p-4 overflow-auto text-xs dark:bg-black">
